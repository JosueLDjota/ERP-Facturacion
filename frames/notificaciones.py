...
class NotificationManager:
    ...
    def shutdown(self):
        """Libera callbacks y ventanas activas antes de cerrar la app."""
        for job_name in ("_queue_processor_job", "_drain_job"):
        ...
        for widget in list(self.notification_widgets):
            try:
                widget._cancel_jobs_for_close()
                if widget.window.winfo_exists():
                    widget.window.destroy()
                widget.is_closed = True
            except Exception:
                logger.exception("Error cerrando notificacion activa durante shutdown.")
        self.notification_widgets.clear()
        self.notification_queue.clear()
        self._queued_or_visible_keys.clear()
